...
enum struct WireFormat {
  ...
  // TODO(apang): Comment why this struct is needed.
...
// TODO(apang): This may not return the aligned size, since e.g. "type->coded_struct.size" below may be unaligned
uint32_t AlignedInlineSize(const fidl_type_t* type, WireFormat wire_format) {
  if (!type) {
    // For integral types (i.e. primitive, enum, bits).
    // TODO(apang): This returns the aligned size... but structs etc below don't return the aligned size :/
    return 8;
  }
  switch (type->type_tag) {
    case fidl::kFidlTypePrimitive:
...
// TODO(apang): Assume out_traversal_result is nonnull
 ...
 protected:
  zx_status_t Transform(const fidl_type_t* type, const Position& position,
                        const uint32_t dst_size, TraversalResult* out_traversal_result) {
    if (!type) {
    ...
    switch (type->type_tag) {
      ...
      case fidl::kFidlTypeStructPointer: {
        ...
        const auto& dst_coded_struct = *src_coded_struct.alt_type;
        return TransformStructPointer(src_coded_struct, dst_coded_struct, position, out_traversal_result);
      }
      case fidl::kFidlTypeUnionPointer:
      ...
      case fidl::kFidlTypeStruct: {
        const auto& src_coded_struct = type->coded_struct;
        const auto& dst_coded_struct = *src_coded_struct.alt_type;
        return TransformStruct(src_coded_struct, dst_coded_struct, position, dst_size, out_traversal_result);
      }
      case fidl::kFidlTypeUnion: {
      ...
      case fidl::kFidlTypeArray: {
        ...
        return TransformArray(src_coded_array, dst_coded_array, position, dst_array_size, out_traversal_result);
      }
      case fidl::kFidlTypeString:
  ...
  no_transform_just_copy:  // TODO(apang): Remove goto here (can be inlined into case)
    ...
    // TODO(apang): Think about putting logic for updating out_traversal_result in Copy/etc functions.
  ...
  zx_status_t TransformStructPointer(const fidl::FidlCodedStruct& src_coded_struct,
    ...
    return TransformStruct(src_coded_struct, dst_coded_struct, struct_position, aligned_dst_size, out_traversal_result);
  }

  zx_status_t TransformStruct(const fidl::FidlCodedStruct& src_coded_struct,
                              const fidl::FidlCodedStruct& dst_coded_struct,
                              const Position& position, uint32_t dst_size, TraversalResult* out_traversal_result) {
    ...
    // copy everything before the first field if it's non-primitive.
    {
      ...
      if (src_first_field.type != nullptr) {
        ...
        // current_position = position.IncreaseDstInlineOffset(dst_first_field.offset);

        current_position = position.IncreaseSrcInlineOffset(src_first_field.offset).IncreaseDstInlineOffset(dst_first_field.offset);
      }
    }
    ...
    for (uint32_t src_field_index = 0; src_field_index < src_coded_struct.field_count;
      ...
      if (!src_field.type) {
        const uint32_t dst_field_size = src_field.padding_offset +
    ...
    if (From() == WireFormat::kOld) {
      uint32_t src_inline_remaining = position.src_inline_offset + src_coded_struct.size - current_position.src_inline_offset;
  ...
  zx_status_t TransformVector(const fidl::FidlCodedVector& src_coded_vector,
    ...
    // Early exit on nullable vectors.  // TODO(apang): Switch() against FIDL_ALLOC_PRESENT and ABSENT (and eveyrwhere elese)
  ...
  zx_status_t TransformTable(const fidl::FidlCodedTable &coded_table,
    ...
    const fidl_envelope_t* envelopes_vector = src_dst->Read<fidl_envelope_t>(envelopes_vector_position);
    ...
    for(uint32_t i = 0, field_index = 0; i < table->envelopes.count; i++) {
      ...
      zx_status_t status = TransformEnvelope(std::make_optional(field.type), envelope_position, &envelope_traversal_result);
  ...
  virtual zx_status_t TransformUnion(const fidl::FidlCodedUnion& src_coded_union,
                                     ...
                                     const Position& position, TraversalResult* out_traversal_result) = 0;
...
class V1ToOld final : public TransformerBase {
 public:
  ...
  zx_status_t TransformUnion(const fidl::FidlCodedUnion& src_coded_union,
    ...
    zx_status_t status = Transform(src_field->type, field_position, dst_field_size, out_traversal_result);
    if (status != ZX_OK) {
      return status;
    }
...
class OldToV1 final : public TransformerBase {
 ...
 private:
  ...
  zx_status_t TransformUnion(const fidl::FidlCodedUnion& src_coded_union,
    ...
    TraversalResult traversal_result;
    // if (field_position.src_inline_offset == 44 && field_position.dst_inline_offset == 80) {
    //   // sandwich6, case8
    // }
    zx_status_t status = Transform(src_field.type, field_position, dst_inline_field_size, &traversal_result);
    if (status != ZX_OK) {
      return status;
    }

    // printf("(%s) traversal result: %u\n", src_coded_union.name, traversal_result.out_of_line_size);
